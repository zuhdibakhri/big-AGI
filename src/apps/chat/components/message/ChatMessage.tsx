...
export function ChatMessage(props: {
  ...
  // external state
  const { adjContentScaling, disableMarkdown, doubleClickToEdit, uiComplexityMode } = useUIPreferencesStore(
    useShallow((state) => ({
  ...
  const isSpecialT2I =
    textSubject.startsWith('https://images.prodia.xyz/') ||
    textSubject.startsWith('/draw ') ||
    textSubject.startsWith('/imagine ') ||
    textSubject.startsWith('/img ');
  ...
  const handleFragmentReplace = React.useCallback(
    (fragmentId: DMessageFragmentId, newFragment: DMessageFragment) => {
  ...
  const handleApplyEdit = React.useCallback(
    (fragmentId: DMessageFragmentId, editedText: string) => {
      if (editedText.length > 0) handleFragmentReplace(fragmentId, createTextContentFragment(editedText));
      else handleFragmentDelete(fragmentId);
    },
    [handleFragmentDelete, handleFragmentReplace],
  );

  const handleApplyAllEdits = React.useCallback(
    async (withControl: boolean) => {
  ...
  const handleEditSetText = React.useCallback(
    (fragmentId: DMessageFragmentId, editedText: string, applyNow: boolean) => {
      if (applyNow) handleApplyEdit(fragmentId, editedText);
      else setTextContentEditState((prev): ChatMessageTextPartEditState => ({ ...prev, [fragmentId]: editedText || '' }));
    },
    [handleApplyEdit],
  );
  ...
  const handleOpsEditToggle = React.useCallback(
    (e: React.MouseEvent) => {
  ...
  const openContextMenu = React.useCallback(
    (event: MouseEvent, selectedText: string) => {
  ...
  const handleContextMenu = React.useCallback(
    (event: MouseEvent) => {
      ...
      if (selection && selection.rangeCount > 0) {
        ...
        if (selectedText.length > 0) openContextMenu(event, selectedText);
      }
    },
    [openContextMenu],
  );
  ...
  const handleOpenBubble = React.useCallback(
    (_event: MouseEvent) => {
      // check for selection
      const selection = window.getSelection();
  ...
  const handleBlocksContextMenu = React.useCallback(
  ...
  const handleBlocksDoubleClick = React.useCallback(
    (event: React.MouseEvent) => {
      if (doubleClickToEdit || event.shiftKey) props.onMessageFragmentReplace && handleOpsEditToggle(event);
    },
    [doubleClickToEdit, handleOpsEditToggle, props.onMessageFragmentReplace],
  );

  const handleBlocksMouseUp = React.useCallback(
    (event: React.MouseEvent) => {
  ...
  return (
    ...
    >
      ...
      <Box role={undefined /* aside | message | ops */} sx={fromAssistant && !isEditingText ? messageBodySx : messageBodyReverseSx}>
        ...
        <Box ref={blocksRendererRef /* restricts the BUBBLE menu to the children of this */} sx={fragmentsListSx}>
          ...
          {props.isBottom && messageGenerator?.tokenStopReason === 'out-of-tokens' && !!props.onMessageContinue && (
            <BlockOpContinue contentScaling={adjContentScaling} messageId={messageId} messageRole={messageRole} onContinue={props.onMessageContinue} />
          )}
        </Box>
      ...
      {!!opsMenuAnchor && (
        <CloseableMenu dense placement="bottom-end" open={true} anchorEl={opsMenuAnchor} onClose={handleCloseOpsMenu} sx={{ minWidth: 280 }}>
          {fromSystem && (
            <ListItem>
            ...
            {!!onMessageToggleUserFlag && (
              <MenuItem onClick={handleOpsToggleStarred} sx={{ flexGrow: 0, px: 1 }}>
                <Tooltip disableInteractive title={!isUserStarred ? 'Star message - use @ to refer to it later' : 'Unstar'}>
                  {isUserStarred ? <StarRoundedIcon color="primary" sx={{ fontSize: 'xl2' }} /> : <StarOutlineRoundedIcon sx={{ fontSize: 'xl2' }} />}
                </Tooltip>
              </MenuItem>
            )}
          </Box>
          ...
          {!messagePendingIncomplete && !isUserMessageSkipped && !!props.showAntPromptCaching && (
            <MenuItem onClick={handleOpsToggleAntCacheUser}>
              <ListItemDecorator>
                <AnthropicIcon sx={isVndAndCacheUser ? antCachePromptOnSx : antCachePromptOffSx} />
              </ListItemDecorator>
              {isVndAndCacheUser ? (
                'Do not cache'
              ) : (
                <>
                  Cache <span style={{ opacity: 0.5 }}>up to here</span>
                </>
              )}
            </MenuItem>
          )}
          {!messagePendingIncomplete && !isUserMessageSkipped && !!props.showAntPromptCaching && isVndAndCacheAuto && !isVndAndCacheUser && (
            <MenuItem disabled>
              <ListItemDecorator>
                <TextureIcon sx={{ color: ModelVendorAnthropic.brandColor }} />
              </ListItemDecorator>
            ...
            <MenuItem disabled={fromSystem} onClick={handleOpsAssistantFrom}>
              ...
              {!fromAssistant ? (
                ...
                <Box sx={{ flexGrow: 1, display: 'flex', justifyContent: 'space-between', gap: 1 }}>
                  Retry
                  <KeyStroke variant="outlined" combo="Ctrl + Shift + Z" />
                </Box>
              )}
            </MenuItem>
          )}
          {!!props.onMessageBeam && (
            <MenuItem disabled={fromSystem} onClick={handleOpsBeamFrom}>
              ...
              {!fromAssistant ? (
                ...
                <Box sx={{ flexGrow: 1, display: 'flex', justifyContent: 'space-between', gap: 1 }}>
                  Beam Edit
                  <KeyStroke variant="outlined" combo="Ctrl + Shift + B" />
                </Box>
              )}
            </MenuItem>
          )}
        ...
        >
          <ClickAwayListener onClickAway={() => closeBubble()}>
            ...
            >
              ...
              {!!onAddInReferenceTo && (
                <Tooltip disableInteractive arrow placement="top" title={props.hasInReferenceTo ? 'Reply to this too' : fromAssistant ? 'Reply' : 'Refer To'}>
              ...
              {fromAssistant && (
                <Tooltip disableInteractive arrow placement="top" title="Highlight Text">
                  <IconButton
                    disabled={!handleHighlightSelText}
                    onClick={
              ...
              {fromAssistant && (
                <Tooltip disableInteractive arrow placement="top" title="Strike Through">
                  <IconButton
                    disabled={!handleHighlightSelText}
                    onClick={
                      !handleHighlightSelText
              ...
              {fromAssistant && (
                <Tooltip disableInteractive arrow placement="top" title="Toggle Bold">
                  <IconButton
                    disabled={!handleHighlightSelText}
                    onClick={
                      !handleHighlightSelText
              ...
              {!!props.onTextDiagram && (
                <Tooltip disableInteractive arrow placement="top" title={couldDiagram ? 'Auto-Diagram...' : 'Too short to Auto-Diagram'}>
              ...
              {!!props.onTextImagine && (
                <Tooltip disableInteractive arrow placement="top" title="Auto-Draw">
              ...
              {!!props.onTextSpeak && (
                <Tooltip disableInteractive arrow placement="top" title="Speak">
      ...
      {!!contextMenuAnchor && (
        <CloseableMenu dense placement="bottom-start" open={true} anchorEl={contextMenuAnchor} onClose={closeContextMenu} sx={{ minWidth: 220 }}>
          ...
          {!!props.onTextDiagram && <ListDivider />}
          {!!props.onTextDiagram && (
            <MenuItem onClick={handleOpsDiagram} disabled={!couldDiagram || props.isImagining}>
              <ListItemDecorator>
                <AccountTreeOutlinedIcon />
              </ListItemDecorator>
              Auto-Diagram ...
