...
} from '~/common/components/useSpeechRecognition';
...
export function Composer(props: {
  ...
  const { chatExecuteMode, chatExecuteModeSendColor, chatExecuteModeSendLabel, chatExecuteMenuComponent, chatExecuteMenuShown, showChatExecuteMenu } =
    useChatExecuteMode(props.capabilityHasT2I, props.isMobile);
  const micCardRef = React.useRef<HTMLDivElement>(null);

  // external state
  const { showPromisedOverlay } = useOverlayComponents();
  const { labsAttachScreenCapture, labsCameraDesktop, labsShowCost, labsShowShortcutBar } = useUXLabsStore(
    useShallow((state) => ({
  ...
  const {
    assistantAbortible,
    systemPurposeId,
    tokenCount: _historyTokenCount,
    abortConversationTemp,
  } = useChatStore(
    useShallow((state) => {
  ...
  const handleFilterAGIFile = React.useCallback(
    async (file: File): Promise<boolean> =>
      await showPromisedOverlay('composer-open-or-attach', { rejectWithValue: false }, ({ onResolve, onUserReject }) => (
        <ConfirmationModal
          ...
          positiveActionText="Open"
          negativeActionText="Attach"
  ...
  const {
    ...
    /* append */ attachAppendClipboardItems,
    attachAppendDataTransfer,
    attachAppendEgoFragments,
    attachAppendFile,
    /* take */ attachmentsRemoveAll,
  ...
  const tokensComposerTextDebounced = React.useMemo(() => {
    return debouncedText && props.chatLLM ? estimateTextTokens(debouncedText, props.chatLLM, 'composer text') : 0;
  ...
  if (props.chatLLM && tokensComposer > 0) tokensComposer += glueForMessageTokens(props.chatLLM);
  ...
  const handleRemoveInReferenceTo = React.useCallback(
  ...
  React.useEffect(() => {
    if (inReferenceTo?.length) setTimeout(() => composerTextAreaRef.current?.focus(), 1 /* prevent focus theft */);
  ...
  const handleSendAction = React.useCallback(
    async (_chatExecuteMode: ChatExecuteMode, composerText: string): Promise<boolean> => {
      ...
      if (composerText) fragments.push(createTextContentFragment(composerText));
  ...
  const handleAppendTextAndSend = React.useCallback(
    async (appendText: string) => {
  ...
  const onActileCommandPaste = React.useCallback(
    (item: ActileItem) => {
  ...
  const onActileEmbedMessage = React.useCallback(
    async ({ conversationId, messageId }: StarredMessageItem) => {
      // get the message
  ...
  const handleTextareaTextChange = React.useCallback(
  ...
  const handleTextareaKeyDown = React.useCallback(
    async (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
      ...
      if (actileInterceptKeydown(e)) return;
      ...
      if (e.key === 'Enter') {
        ...
        if (e.shiftKey) touchShiftEnter();
        if (enterIsNewline ? e.shiftKey : !e.shiftKey) {
          if (!assistantAbortible) await handleSendAction(chatExecuteMode, composeText); // enter -> send
          return e.preventDefault();
        }
      }
    },
  ...
  const micColor: ColorPaletteProp = recognitionState.errorMessage
    ? 'danger'
  ...
  React.useEffect(() => {
    ...
    if (micContinuationTrigger) toggleRecognition();
  ...
  const handleAttachCtrlV = React.useCallback(
    async (event: React.ClipboardEvent) => {
      if ((await attachAppendDataTransfer(event.clipboardData, 'paste', false)) === 'as_files') event.preventDefault();
    },
    [attachAppendDataTransfer],
  );

  const handleAttachCameraImage = React.useCallback(
    (file: FileWithHandle) => {
  ...
  const handleAttachScreenCapture = React.useCallback(
    (file: File) => {
  ...
  const handleAttachFiles = React.useCallback(
    async (files: FileWithHandle[], errorMessage: string | null) => {
      if (errorMessage) addSnackbar({ key: 'attach-files-open-fail', message: `Unable to open files: ${errorMessage}`, type: 'issue' });
      for (let file of files)
        await attachAppendFile('file-open', file).catch((error: any) =>
          addSnackbar({
            key: 'attach-file-open-fail',
            message: `Unable to attach the file "${file.name}" (${error?.message || error?.toString() || 'unknown error'})`,
            type: 'issue',
          }),
        );
    },
  ...
  const handleAttachmentDraftsAction = React.useCallback(
    (attachmentDraftIdOrAll: AttachmentDraftId | null, action: LLMAttachmentDraftsAction) => {
      switch (action) {
        case 'copy-text':
    ...
    React.useMemo(() => {
      ...
      if (recognitionState.isActive) {
        composerShortcuts.push({
          key: 'm',
          ctrl: true,
          action: () => toggleRecognition(true),
          description: 'Mic · Send',
          disabled: !recognitionState.hasSpeech,
          endDecoratorIcon: TelegramIcon as any,
          level: 4,
        });
        composerShortcuts.push({
          key: ShortcutKey.Esc,
  ...
  const sendButtonColor: ColorPaletteProp = assistantAbortible
    ? 'warning'
    : !llmAttachmentDraftsCollection.canAttachAllFragments
      ? 'warning'
      : chatExecuteModeSendColor;

  const sendButtonLabel = chatExecuteModeSendLabel;

  const sendButtonIcon = micContinuation ? null : isAppend ? (
  ...
  const beamButtonColor: ColorPaletteProp | undefined = !llmAttachmentDraftsCollection.canAttachAllFragments ? 'warning' : undefined;
  ...
  let textPlaceholder: string = 'Start typing...';
                    ...
                    >
                      {isDesktop && (
                        <ButtonMicMemo
                          variant={micVariant}
                          color={micColor}
                          errorMessage={recognitionState.errorMessage}
                          onClick={handleToggleMic}
                          noBackground={!recognitionState.isActive}
                        />
                      )}
                      ...
                      {micIsRunning && (
                        <ButtonMicContinuationMemo
                          ...
                          variant={micContinuation ? 'soft' : 'soft'}
                          color={micContinuation ? 'primary' : 'neutral'}
                          sx={{ background: micContinuation ? undefined : 'none' }}
                  ...
                  {micIsRunning && (
                    ...
                    >
                      ...
                      >
                        ...
                        <span className={speechInterimResult.interimTranscript === PLACEHOLDER_INTERIM_TRANSCRIPT ? 'placeholder' : 'interim'}>
