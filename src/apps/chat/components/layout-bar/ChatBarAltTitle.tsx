...
export function ChatBarAltTitle(props: { conversationId: DConversationId | null; conversationTitle: string }) {
  // state
  ...
  return (
    <Box sx={{ display: 'flex', gap: { xs: 1, md: 3 }, alignItems: 'center' }}>
      <Typography>{capitalizeFirstLetter(conversationTitle?.trim() || CHAT_NOVEL_TITLE)}</Typography>
