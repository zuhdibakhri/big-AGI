...
export const useFolders = (activeFolderId: string | null) =>
  useFolderStore(
    useShallow(({ enableFolders, folders, toggleEnableFolders }) => {
      // finds the active folder if any
      const activeFolder = enableFolders && activeFolderId ? (folders.find((folder) => folder.id === activeFolderId) ?? null) : null;
...
function ChatDrawer(props: {
  ...
  const {
    clearFilters,
    filterHasDocFragments,
    toggleFilterHasDocFragments,
    filterHasImageAssets,
    toggleFilterHasImageAssets,
    filterHasStars,
    toggleFilterHasStars,
    showPersonaIcons,
    toggleShowPersonaIcons,
    showRelativeSize,
    toggleShowRelativeSize,
  } = useChatDrawerFilters();
  const { activeFolder, allFolders, enableFolders, toggleEnableFolders } = useFolders(props.activeFolderId);
  const { filteredChatsCount, filteredChatIDs, filteredChatsAreEmpty, filteredChatsBarBasis, filteredChatsIncludeActive, renderNavItems } =
    useChatDrawerRenderItems(
  ...
  const handleConversationActivate = React.useCallback(
    (conversationId: DConversationId, closeMenu: boolean) => {
      onConversationActivate(conversationId);
      if (closeMenu && getIsMobile()) optimaCloseDrawer();
    },
  ...
  const handleConversationDeleteNoConfirmation = React.useCallback(
    ...
    <>
      ...
      {!!folderChangeRequest?.anchorEl && (
        <CloseableMenu
          bigIcons
          open
          anchorEl={folderChangeRequest.anchorEl}
          onClose={handleConversationFolderCancel}
