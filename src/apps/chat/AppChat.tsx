...
import {
  ...
  duplicateDMessageFragmentsNoPH,
...
import {
  createDMessageFromFragments,
  createDMessagePlaceholderIncomplete,
  DMessageMetadata,
  duplicateDMessageMetadata,
} from '~/common/stores/chat/chat.message';
...
export const CHAT_NOVEL_TITLE = 'New Chat';
...
export function AppChat() {
  ...
  const handleOpenConversationInFocusedPane = React.useCallback(
  ...
  const handleOpenConversationInSplitPane = React.useCallback(
    (conversationId: DConversationId | null) => {
  ...
  const handleNavigateHistoryInFocusedPane = React.useCallback(
    (direction: 'back' | 'forward') => {
      if (navigateHistoryInFocusedPane(direction)) showNextTitleChange.current = true;
    },
    [navigateHistoryInFocusedPane],
  );
  ...
  const handleExecuteAndOutcome = React.useCallback(async (chatExecuteMode: ChatExecuteMode, conversationId: DConversationId, callerNameDebug: string) => {
    ...
    if (outcome === 'err-no-chatllm') optimaOpenModels();
    else if (outcome === 'err-t2i-unconfigured') optimaOpenPreferences('draw');
    else if (outcome === 'err-no-persona') addSnackbar({ key: 'chat-no-persona', message: 'No persona selected.', type: 'issue' });
    else if (outcome === 'err-no-conversation') addSnackbar({ key: 'chat-no-conversation', message: 'No active conversation.', type: 'issue' });
    else if (outcome === 'err-no-last-message') addSnackbar({ key: 'chat-no-conversation', message: 'No conversation history.', type: 'issue' });
    return outcome === true;
  }, []);

  const handleComposerAction = React.useCallback(
    (
      ...
      const uniqueConversationIds = willMulticast ? Array.from(new Set([conversationId, ...paneUniqueConversationIds])) : [conversationId];

      // validate conversation existence
      const uniqueConverations = uniqueConversationIds.map((cId) => getConversation(cId)).filter(Boolean) as DConversation[];
      if (!uniqueConverations.length) return false;
  ...
  const handleConversationExecuteHistory = React.useCallback(
    async (conversationId: DConversationId) => {
  ...
  const handleMessageRegenerateLastInFocusedPane = React.useCallback(async () => {
    ...
    if (focusedPaneConversationId && focusedConversation?.messages?.length) {
      ...
      if (lastMessage.role === 'assistant') ConversationsManager.getHandler(focusedPaneConversationId).historyTruncateTo(lastMessage.id, -1);
  ...
  const handleMessageBeamLastInFocusedPane = React.useCallback(async () => {
    ...
    if (focusedConversation?.messages?.length) {
      ...
      else if (lastMessage.role === 'user') ConversationsManager.getHandler(focusedConversation.id).beamInvoke(focusedConversation.messages, [], null);
    }
  }, [focusedPaneConversationId]);

  const handleTextDiagram = React.useCallback((diagramConfig: DiagramConfig | null) => setDiagramConfig(diagramConfig), []);

  const handleImagineFromText = React.useCallback(
    async (conversationId: DConversationId, subjectText: string) => {
      ...
      await imaginePromptFromTextOrThrow(subjectText, conversationId)
        ...
        .catch((error: any) => {
          ...
          cHandler.messageFragmentReplace(
            userImagineMessage.id,
            userImagineMessage.fragments[0].fId,
            createErrorContentFragment(`Issue requesting an Image prompt. ${error?.message || ''}`),
            true,
          );
        });
    },
  ...
  const handleConversationNewInFocusedPane = React.useCallback(
    (forceNoRecycle?: boolean) => {
      // create conversation (or recycle the existing top-of-stack empty conversation)
      const conversationId =
        recycleNewConversationId && !forceNoRecycle
          ? recycleNewConversationId
      ...
      if (activeFolderId && conversationId) useFolderStore.getState().addConversationToFolder(activeFolderId, conversationId);
  ...
  const handleFileSaveConversation = React.useCallback((conversationId: DConversationId | null) => {
    ...
    conversation &&
      downloadSingleChat(conversation, 'json')
        ...
        .catch((err: any) => {
          if (err?.name !== 'AbortError') addSnackbar({ key: 'chat-save-as-fail', message: `Could not save the file. ${err?.message || ''}`, type: 'issue' });
        });
  }, []);

  const handleConversationBranch = React.useCallback(
    (srcConversationId: DConversationId, messageId: string | null, addSplitPane: boolean): DConversationId | null => {
      // clone data
      const branchedConversationId = branchConversation(srcConversationId, messageId);

      // if a folder is active, add the new conversation to the folder
      if (activeFolderId && branchedConversationId) useFolderStore.getState().addConversationToFolder(activeFolderId, branchedConversationId);

      // replace/open a new pane with this
      showNextTitleChange.current = true;
      if (addSplitPane && isMultiAddable) handleOpenConversationInSplitPane(branchedConversationId);
      else handleOpenConversationInFocusedPane(branchedConversationId);
  ...
  const handleConversationReset = React.useCallback(
    async (conversationId: DConversationId) => {
      if (
        await showPromisedOverlay('chat-reset-confirmation', { rejectWithValue: false }, ({ onResolve, onUserReject }) => (
          <ConfirmationModal
            open
  ...
  const handleDeleteConversations = React.useCallback(
    async (conversationIds: DConversationId[], bypassConfirmation: boolean) => {
      // show confirmation dialog
      if (
        !bypassConfirmation &&
        !(await showPromisedOverlay('chat-delete-confirmation', { rejectWithValue: false }, ({ onResolve, onUserReject }) => (
          <ConfirmationModal
            open
            onClose={onUserReject}
            onPositive={() => onResolve(true)}
  ...
  const focusedBarContent = React.useMemo(
    () =>
    ...
    React.useMemo(
      () => [
        ...
        { key: 'n', ctrl: true, shift: true, action: handleConversationNewInFocusedPane },
        {
          ...
          ctrl: true,
          shift: true,
          action: () => isFocusedChatEmpty || (focusedPaneConversationId && handleConversationReset(focusedPaneConversationId)),
        },
      ...
      [
        ...
        handleConversationNewInFocusedPane,
        handleDeleteConversations,
        handleConversationsImportFormFilePicker,
        handleFileSaveConversation,
        handleMessageBeamLastInFocusedPane,
        handleMessageRegenerateLastInFocusedPane,
        handleNavigateHistoryInFocusedPane,
  ...
  return (
    <>
      ...
      <PanelGroup direction={isMobile || isTallScreen ? 'vertical' : 'horizontal'} id="app-chat-panels">
        {chatPanes.map((pane, idx) => {
          ...
          return (
            <WorkspaceIdProvider conversationId={_paneIsFocused ? _paneConversationId : null} key={_keyAndId}>
              <Panel
                id={_keyAndId}
                order={idx}
                ...
                style={{
                  ...
                  ...(isMultiPane
                    ? {
                        ...
                        border: `2px solid ${
                          _paneIsFocused
                            ? willMulticast || !isMultiConversationId
              ...
              >
                <ScrollToBottom bootToBottom stickToBottomInitial sx={scrollToBottomSx}>
                  {!_paneBeamIsOpen && (
                    <ChatMessageList
                  ...
                  {_paneBeamIsOpen && <ChatBeamWrapper beamStore={_paneBeamStoreApi} isMobile={isMobile} inlineSx={chatBeamWrapperSx} />}
      ...
      {!!flattenConversationId && (
        <FlattenerModal conversationId={flattenConversationId} onConversationBranch={handleConversationBranch} onClose={() => setFlattenConversationId(null)} />
      )}

      {/* Import / Export  */}
      {!!tradeConfig && <TradeModal config={tradeConfig} onConversationActivate={handleOpenConversationInFocusedPane} onClose={() => setTradeConfig(null)} />}
    </>
  );
}
