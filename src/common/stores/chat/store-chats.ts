  ...
  persist(
    (_set, _get) => ({
      ...
      deleteConversations: (conversationIds: DConversationId[], newConversationPersonaId?: SystemPurposeId): DConversationId => {
        ...
        if (!newConversations.length) newConversations.push(createDConversation(newConversationPersonaId));
        ...
        _get()._editConversation(conversationId, ({ _abortController: _currentController }) => {
          ...
          if (_nextController !== null && _currentController) {
            ...
            if (process.env.NODE_ENV === 'development')
              console.warn(
                `[DEV] setAbortController (${debugScope}): race condition (${isAlreadyAborted ? 'Already aborted' : 'Not aborted'}) for conversation ${conversationId}`,
              );
            if (!isAlreadyAborted) _currentController.abort();
          }
          return {
      ...
      historyTruncateToIncluded: (conversationId: DConversationId, messageId: DMessageId, offset: number) =>
        _get()._editConversation(conversationId, (conversation) => {
          ...
          if (messageIndex < 0 || messageIndex + 1 + offset >= conversation.messages.length) return {};
      ...
      appendMessage: (conversationId: DConversationId, message: DMessage) =>
        _get()._editConversation(conversationId, (conversation) => {
          ...
          if (!message.pendingIncomplete) updateMessagesTokenCounts([message], true, 'appendMessage');
      ...
      editMessage: (
        ...
        _get()._editConversation(conversationId, (conversation) => {
          const messages = conversation.messages.map((message): DMessage => {
            ...
            if (removePendingState) delete updatedMessage.pendingIncomplete;

            if (!updatedMessage.pendingIncomplete) updateMessageTokenCount(updatedMessage, getChatLLMId(), true, 'editMessage(incomplete=false)');
    ...
    }),
    {
      ...
      migrate: async (state: any, fromVersion: number) => {
        ...
        if (fromVersion < 4 && state && state.conversations && state.conversations.length) {
          if (await backupIdbV3('app-chats', 'app-chats-v3')) console.warn('Migrated app-chats from v3 to v4');
...
export const useConversation = (conversationId: DConversationId | null) =>
  useChatStore(
    useShallow((state) => {
