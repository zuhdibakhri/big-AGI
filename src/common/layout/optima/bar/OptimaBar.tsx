    ...
    <>
        ...
        <InvertedBarCornerItem>
          <IconButton
            ...
            onClick={
              menuToPanel
                ? optimaTogglePanel
                : optimaOpenAppMenu /* onPointerDown doesn't work well with a menu (the 'up' event would close it), so we're still with onClick */
            }
            onContextMenu={
              menuToPanel ? optimaOpenPanel : optimaOpenAppMenu /* important to get the 'preventDefault' for the Right mouse click (to prevent the menu) */
            }
      ...
      {menuToPanel ? (
        ...
        <CloseableMenu
          ...
          noBottomPadding={props.isMobile}
          placement="bottom-end"
          open={isAppMenuOpen && !!appMenuAnchor.current}
          anchorEl={appMenuAnchor.current}
          onClose={optimaCloseAppMenu}
          sx={{ minWidth: 280 }}
        >
          {/* Common (Preferences) */}
          {commonAppMenuItems}
