...
export function makeMessageAvatarIcon(
  ...
  switch (messageRole) {
    ...
    case 'user':
      return <PersonPinIcon sx={avatarIconSx} />; // https://www.svgrepo.com/show/306500/openai.svg
      ...
      tooltip:
        complexity === 'minimal' ? null : (
          <Box sx={tooltipSx}>
            ...
            {complexity === 'extra' && !!created && (
              <Box sx={tooltipCreationTimeSx}>
                {updated ? 'Updated' : 'Created'} <TimeAgo date={updated || created} />.
              </Box>
            )}
          </Box>
        ),
...
export function prettyShortChatModelName(model: string | undefined): string {
  ...
  if (model.includes('gpt-4')) {
    ...
    if (model.includes('gpt-4-0125-preview') || model.includes('gpt-4-1106-preview') || model.includes('gpt-4-turbo')) return 'GPT-4 Turbo';
